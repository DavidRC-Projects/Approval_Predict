...
def plot_numerical(df, vars_to_study, target_var):
    ...
    for col in vars_to_study:
        ...
        axes[0].set_title(
            f"{col} distribution by {target_var}"
        )
...
def page_feature_impact_body():
    ...
    (numerical plots, heatmap, and parallel categories)
    for loan approval patterns.
    """
    try:
        ...
        st.write(
            f"* The dataset has {df_preview.shape[0]} rows and "
            f"{df_preview.shape[1]} columns."
        )
        vars_to_study = [
            "points",
            "credit_score",
        ...
        with st.expander("Summary of numerical feature insights below"):
            ...
            st.write(
                ...
                higher ratios and outliers indicate financial risk and more
                frequent rejections.
                """
            )

        df_eda = df_preview[vars_to_study + ['loan_approved']]
        ...
        with st.expander("Show summary of correlation heatmap insights"):
            st.write(
                """
        ...
        with st.expander("Show summary of parallel categories insights"):
            st.write(
                """
        ...
        corr_spearman = (
            df_preview.corr(method="spearman")["loan_approved"]
            .sort_values(key=abs, ascending=False)[1:]
            .head(5)
        )

        corr_pearson = (
            df_preview.corr(method="pearson")["loan_approved"]
            .sort_values(key=abs, ascending=False)[1:]
            .head(5)
        )
        ...
        st.info(
            ...
            - Applicants with **higher credit scores** are more likely to be
            approved.
            - Applicants with **low points** and **low credit scores** show a
            strong likelihood of rejection.
            - **Income**, **loan amount**, and **years employed** show weaker
            relationships with approval outcomes.
            - The parallel categories analysis highlights a strong link between
            **loan-to-income ratio** and loan approval decisions.
            """
        )
